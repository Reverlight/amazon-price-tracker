...
st.markdown(
    f"<p style='color:#94a3b8;font-size:13px;margin-top:-12px'>Auto-refreshes every 5 minutes &nbsp;·&nbsp; Last loaded: {datetime.utcnow().strftime('%H:%M:%S')} UTC</p>",
    unsafe_allow_html=True,
)
...
table_df["Last Checked"] = pd.to_datetime(table_df["Last Checked"]).dt.strftime(
    "%b %d  %H:%M"
)
...
table_df["24h Change"] = table_df["24h Change"].map(
    lambda v: f"▲ {v:.1f}%" if v > 0 else (f"▼ {abs(v):.1f}%" if v < 0 else "— 0.0%")
)
...
if selected:
    ...
    filtered = (
        product_data[product_data["timestamp"] >= cutoff]
        if days < 9999
        else product_data
    )
